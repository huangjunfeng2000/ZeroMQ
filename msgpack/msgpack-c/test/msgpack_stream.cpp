...
#define GEN_TEST_STREAM(test_type)                                      \
  for (unsigned int k = 0; k < kLoop; k++) {                            \
    ...
    vec_type vec;   size_t iLoopCount = 1000;                                                    \
    for(unsigned int i = 0; i < iLoopCount/*rand() % kLoop*/; ++i) {                  \
	...
	if (k==0)	printf("             %d  %d\n", vec.size() * sizeof(test_type), sbuf.size());\
    /*break;*/\
	while (p < pend) {                                                  \
      size_t sz = std::min<size_t>(pend - p, rand() % 128);       \
	  sz = (pend - p);       \
