...
TEST(unpack, sequence)
{
    ...
    msgpack::pack(sbuf, 2);
    msgpack::pack(sbuf, 3);
    msgpack::pack(sbuf, 4.0f);
    msgpack::pack(sbuf, 5.0);
	msgpack::pack(sbuf, "abc");
	...
	msgpack::unpack(msg, sbuf.data(), sbuf.size(), off);
	EXPECT_EQ(4, msg.get().as<float>());

	msgpack::unpack(msg, sbuf.data(), sbuf.size(), off);
	EXPECT_EQ(5, msg.get().as<double>());


	msgpack::unpack(msg, sbuf.data(), sbuf.size(), off);
	EXPECT_EQ("abc", msg.get().as<std::string>());
