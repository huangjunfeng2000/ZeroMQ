...
kvmsg_sequence (kvmsg_t *self)
{
    ...
    if (self->present [FRAME_SEQ]) {
        ...
        byte *source = (byte *)zmq_msg_data (&self->frame [FRAME_SEQ]);
...
kvmsg_set_sequence (kvmsg_t *self, int64_t sequence)
{
    ...
    byte *source = (byte *)zmq_msg_data (msg);
...
kvmsg_set_prop (kvmsg_t *self, char *name, char *format, ...)
{
    ...
    char *prop = (char *)malloc (strlen (name) + strlen (value) + 2);

    //  Remove existing property if any
    sprintf (prop, "%s=", name);
    char *existing = (char *)zlist_first (self->props);
    while (existing) {
        ...
        existing = (char *)zlist_next (self->props);
    }
