...
public:
   ...
   //  Constructor
    ...
    GDSmdwrk (std::string broker, std::string service, int verbose)
    {
        ...
        s_catch_signals ();
		m_projectInfo.SetProjectName(m_service);
        connect_to_broker ();
    }
    ...
    void send_to_broker(char *command, std::string option, zmsg *_msg)
    {
        ...
        if (m_verbose) {
			if (strcmp(MDPW_HEARTBEAT, command) != 0)
			{
				s_console ("I: sending %s to broker: %s",
				    mdps_commands [(int) *command], msg->body());
				//msg->dump ();
			}
      }
        msg->send (*m_worker);
    ...
    void connect_to_broker ()
    {
		...
		//CMsgProjectInfo projectInfo(m_service);
		zmsg * request = new zmsg(m_projectInfo.ToString().c_str());

        //  Register service with broker
        send_to_broker ((char*)MDPW_READY, m_projectInfo.ServiecName(), request);
    ...
    recv (zmsg *&reply_p)
    {
        ...
        if (reply) {
			...
			if (JSON_Prase(strInfo, rootNode))
			{
				...
				{
				case GMT_C_EDIT_DB:
				case GMT_C_DELETE_DB:
					{
						CMsgEditDB editDB;
						if (editDB.ReadFromJsonNode(rootNode))
						{
							int iType = editDB.m_iType;
							int iID = editDB.m_iID;
							std::string strUUID = editDB.m_strUUID;
							bResult = m_DBManager.Process(editDB);

							CMsgResult msgResult(bResult ? GMT_S_SUCCESS : GMT_S_FAILURE, bResult ? "" : "Lock failure!");
							reply->body_set(msgResult.ToString().c_str());
						}
					}
					break;
				case GMT_C_ADD_DB:
					{
						CMsgAddDB addDB;
						if (addDB.ReadFromJsonNode(rootNode))
						{
							bResult = m_DBManager.Process(addDB);
			...
			if (bResult)	//
			{
				send_to_broker ((char*)MDPW_PUBLISH, strInfo, NULL);
			}			
			delete reply_p;
			reply_p = 0;
        }
        ...
        while (!s_interrupted) {
            ...
            if (items [0].revents & ZMQ_POLLIN) {
                ...
                if (m_verbose) {
					std::string command = (char *)msg->body();
					if (command.compare (MDPW_HEARTBEAT) != 0)
						s_console ("I: received msg bro: %s", msg->body());
                    //msg->dump ();
                }
                m_liveness = HEARTBEAT_LIVENESS;
                ...
                if (command.compare (MDPW_REQUEST) == 0) {
					...
					if (m_verbose) {
						//s_console ("I: received message from broker: heartbeats");
					}
                }
