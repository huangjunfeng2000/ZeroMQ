...
{
	//static long m_dwRefCount = 1000;
	//InterlockedIncrement(&m_dwRefCount);
    std::stringstream ss;
	//ss << std::hex << std::uppercase
	//   << std::setw(4) << std::setfill('0') << m_dwRefCount << "-"
	//   << std::setw(4) << std::setfill('0') << m_dwRefCount+1;
    ss << std::hex << std::uppercase
	...
	memset(str, 0, 100);
	size_t iSize = 0;
	socket.getsockopt(ZMQ_IDENTITY, str, &iSize);
    socket.setsockopt(ZMQ_IDENTITY, ss.str().c_str(), ss.str().length());
	//std::cout << "id: " << ss << std::endl;
    return ss.str();
}
...
s_clock (void)
{
    ...
    //SYSTEMTIME st;
    //GetSystemTime (&st);
    //return (int64_t) st.wMinute * 60 * 1000 + (int64_t) st.wSecond * 1000 + st.wMilliseconds;
	return clock();
#else
	assert(false);
	struct timeval tv;
