...
public:
   ...
   bool recv(zmq::socket_t & socket) {
      ...
      while(1) {
         ...
         ustring data((unsigned char*) message.data(), message.size()+1);
		 assert(data.size() == message.size()+1);
         //std::cerr << "recv: \"" << (unsigned char*) message.data() << "\", size " << message.size() << std::endl;
         if ((/*message.size() == 5 || */message.size() == 17) && ((unsigned char *)message.data())[0] == 0) {
            char *uuidstr = encode_uuid((unsigned char*) message.data());
            push_back(uuidstr);
            delete[] uuidstr;
         }
         else {
			 //char *res =  (char*)message.data();
			 //int iMsgSize = message.size();
